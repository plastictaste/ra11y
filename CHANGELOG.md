...
- **`plan.mechanicalEditsAvailable` renamed to `plan.safeEditsAvailable`** (MCP `scan` / `scan_file` / `scan_project` / `scan_diff` / `bootstrap` response `plan` block; MCP `bootstrap` response `scan` subset; CLI `--format agent` response `plan` block; the `FixCounts.mechanicalEditsAvailable` property on the exported `countFixes` helper under `src/output/agent-response/build-plan.ts`). The counter's tally was unchanged — it still counts violations whose `fixPaths.primary.edit` is populated — but the former name advertised one remediation lane ("mechanical") while always including a second (`verify-in-source`). Field reports surfaced the mismatch: `plan.mechanicalEditsAvailable: 14` co-occurred with `plan.fixesByClass.mechanical: 0`, because every editable violation in that scan routed through the `verify-in-source` lane. Per [`docs/kb/architecture/ai-first-consumer.md`](./docs/kb/architecture/ai-first-consumer.md) "Composite headline counts are dishonest," a top-level counter must name what it actually measures; `safeEditsAvailable` honestly covers the two lanes whose remediation lands in source (mechanical + verify-in-source), which is the "apply_fix can batch this" work the agent budgets against. Per CLAUDE.md §12 this is a minor bump while the plugin API is pre-v1. Consumers that read `plan.mechanicalEditsAvailable` switch to `plan.safeEditsAvailable` (identical value); consumers that need only the mechanical-lane subset read `plan.fixesByClass.mechanical` instead.
- **MCP `bootstrap` response `scan` subset drops `totalFindings`; adds `violationsCount` + `notesCount` + `safeEditsAvailable` + `fixesByClass`** (`src/mcp/tool-bootstrap.ts` `ScanSubset`). Upstream `scan_project` intentionally splits its plan block into `violations` / `notes` / `safeEditsAvailable` / `fixesByClass` (one counter per remediation lane); the bootstrap composer used to re-sum them back into a single `scan.totalFindings` headline, which re-introduced the composite-headline anti-pattern CLAUDE.md §1 warns against — agents budgeting against the sum treated info-severity notes as identical work to violations, and could not tell editable-lane violations apart from guidance/runtime-only lanes without a second tool call. The subset now forwards the upstream split verbatim: `violationsCount` (severity `error` / `warning`), `notesCount` (severity `info`), `safeEditsAvailable` (violations with inline `fixPaths.primary.edit`; present-when-meaningful), and `fixesByClass: { mechanical, guidance, runtimeOnly, verifyInSource }` (present when violations > 0, mirroring the upstream `emitFixesByClass` gate). The `nextStep` prose reads "N violations" (and optionally "+ M notes") instead of "N findings"; `nextStepStructured` routes off `violationsCount` rather than the composite. Consumers that read `response.scan.totalFindings` read `response.scan.violationsCount + response.scan.notesCount` for the former sum, or branch on `violationsCount` alone for the work-budget question `nextStepStructured` answers.
