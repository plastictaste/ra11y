...
- **MCP `scan_file` now classifies build artifacts and emits the same scan-time warnings as `scan_project`** (`src/mcp/tool-scan-file.ts`). Previously `scan_file` on a `.min.css` (or any other minified / build-artifact-shaped input) returned findings without surfacing `meta.scannedBuildArtifacts`, `scanned_build_artifacts_present`, or `scanned_minified_file` — while `scan_project` on the identical file flagged it as a build artifact and emitted both warning codes. Cross-tool routing inconsistency: an agent calling `scan_file` first got a "real-source" picture of a minified vendor file, while the same agent calling `scan_project` would have routed the file into the build-artifact lane. Per [`docs/kb/architecture/ai-first-consumer.md`](./docs/kb/architecture/ai-first-consumer.md) §"Cross-surface count invariant" (warning-telemetry analogue), warnings whose evidence is the file's own content (build-artifact classification, minified-shape detection) must reach every consumer of that file. The handler now overlays the canonical [`buildScanTimeWarnings`](./src/mcp/scan-time-warnings.ts) helper output on top of the assembler-internal warning channel — same helper `coverage` and `checklist` already route through — so the warning code set and `meta.scannedBuildArtifacts` payload agree across every project-rooted scan tool on identical input. Discovery-only codes (`text_source_skipped`, `binary_assets_skipped`, `sourcemap_files_excluded`) intentionally stay omitted on `scan_file`: their predicate reads off `analysisCoverage.skippedByExtension`, which a single-file scan never populates.
- **MCP `coverage` splits the `failingAutomatedCriteria` lump by emission severity** (`src/mcp/tool-coverage.ts`). The field historically listed every criterion with at least one non-info emission against it, conflating an `error`-severity blocker (canonical `color/meaning-by-color-only` fail) with a `warning`-severity nudge under one label whose name reads as test-runner fail/pass. Per [`docs/kb/architecture/ai-first-consumer.md`](./docs/kb/architecture/ai-first-consumer.md) §"Heuristic-mislabeled meta sub-fields are dishonest" the honest-label rule applies to top-level fields too — the discriminator info should not be flattened. The single `failingAutomatedCriteria` field now ships only criteria with ≥1 `error`-severity emission; criteria whose emissions were all `warning`-severity ride alongside under the new `warningAutomatedCriteria` field. The two arrays partition the legacy lump (no overlap, no orphans) so a caller summing `failingAutomatedCriteria.length + warningAutomatedCriteria.length` recovers the old count exactly. The internal `PerStandardCoverage.failingCriteria` shape is unchanged — `certification.ts` and the CLI `coverage` command consume it as a union-of-blockers count by design (a warning-severity finding still surfaces as a blocking item there). Cross-surface invariant locked in by the existing `coverage`-vs-`scan_project` integration test, extended to assert disjointness of the two lanes and union-equality with the scan's fired-and-scoped set.
- **MCP `checklist.summary.automatedCoverage` drops the composite `automatedCriteriaPassRate` for two non-overlapping counters** (`src/mcp/tool-checklist.ts`). The lone scalar bundled "rule fired clean" (`clean`) with "rule never had eligible inputs" (`untestable`) with "rule found violations" (`withFindings`) into one ratio — the canonical "Composite headline counts are dishonest" failure mode in [`docs/kb/architecture/ai-first-consumer.md`](./docs/kb/architecture/ai-first-consumer.md). An agent budgeting against `automatedCriteriaPassRate: 45` couldn't tell which sub-bucket sank the number; on a Tailwind-pre-build / vendor-bundle scan the rate read as failure when the truth was "most rules never had anything to look at." The field is replaced with two non-overlapping counters: `criteriaWithRulesAllClean` (rules ran on eligible input and emitted zero findings — maps to coverage's `clean`) and `criteriaWithoutEligibleInputs` (rules declared extension eligibility but the scan saw no applicable input — maps to coverage's `untestable`). The agent reads both separately and never sums them; if a single ratio is wanted, the structured `meta.perRuleCoverage` array (already on every response) carries the per-rule `coverageConfidence` + `findingsEmitted` discriminator. The full per-standard block (`criteriaTotal`, `criteriaAutomatable`, `criteriaEvaluated`, `automatedCriteriaPassRate`) stays canonical on the `coverage` tool — the composite is honest there alongside the four-counter split (V1-CHECKLIST-AUTOMATED-PASS-RATE-SEMANTICS-UNCLEAR was the closest paired item; this closure also subsumes that). Regression-guard pattern `automated-coverage-pass-rate-composite` added to `scripts/check-response-assembly.ts` so any future re-introduction of `automatedCriteriaPassRate` inside an `automatedCoverage:` literal flowing through `textResult` / `errorResult` fails CI. Closes Q7-CHECKLIST-PASS-RATE-COMPOSITE.
- **MCP `checklist` drops the degenerate `summary.byPriority` counter and narrates `maxCandidatesPerCriterion` clamps** (`src/mcp/tool-checklist.ts`). `summary.byPriority: { high, medium, low }` shipped `{ high: N, medium: 0, low: 0 }` on every corpus — `priorityFor()` returned `"high"` for every A/AA criterion, so the field was a composite that could not disagree (V1-CHECKLIST-PRIORITY-AXIS-DEGENERATE). The per-item `confidence` axis already carries the honest signal (highest-candidate confidence, or `"low"` for bare-criterion items); the dropped summary counter was pure noise. Separately, caller-supplied `maxCandidatesPerCriterion` values outside `[1, 100]` were silently clamped with no response signal — an agent asking for `500` got `100` back indistinguishable from one that asked for `100` (V1-CHECKLIST-MAX-CANDIDATES-PER-CRITERION-CLAMP). The handler now emits `warnings: ["max_candidates_per_criterion_clamped"]` + paired `warningsDetails.max_candidates_per_criterion_clamped: { requested, applied }` whenever the clamp fires. Surface-don't-suppress / honest-shape per `docs/kb/architecture/ai-first-consumer.md`.
- **MCP `sessionConfigure` rejects non-boolean `allowWrite` instead of silently dropping it** (`src/mcp/tools-helpers.ts` `buildConfigureOpts`, `src/mcp/tools.ts`). Previously a caller that sent `allowWrite: "true"` (string) or `allowWrite: 1` (number) slipped past the handler's `typeof … === "boolean"` guard; the field was silently discarded, the session's write gate stayed at its prior value, and the response echoed `active.allowWrite: false` — indistinguishable from "I never asked to flip the gate" on the response side. The canonical "silent drops are dishonest" / ambiguous-success failure mode the AI-first consumer doctrine warns against (`docs/kb/architecture/ai-first-consumer.md`). `buildConfigureOpts` now returns a discriminated `{ ok, opts } | { ok: false, error }` result; the handler surfaces a structured `invalid-param` error with `details.param: "allowWrite"` and remediation prose when the field is present with a non-boolean JSON type. Boolean-typed calls (with or without `cwd`) persist cleanly; omitting the field leaves the existing session setting untouched; `active.allowWrite` in the success response always matches the session's post-call state. Closes V1-SESSION-CONFIGURE-ALLOWWRITE-SILENT-DROP.
- **TSX parser rejects two false JSX contexts that silently dropped downstream rule coverage** (`src/input/parsers/tsx.ts`). (a) JSX attribute expressions whose body contained a template-literal — or string-literal, or comment — with an embedded `}` used to close the expression early via a naive depth-only `#skipBraceBlock`; the parser then re-entered JSX-child mode mid-template-content, found orphan HTML-shaped tags, and emitted `Unclosed JSX element <Example>` (or <iframe>/<body>/<span> on Bootstrap's Astro/Starlight docs — `src/content/docs/helpers/ratio.mdx:22`, `components/dropdowns.mdx:384` and 10 sibling files). `#skipBraceBlock` now skips over those spans exactly like the top-level scanner, so their inner `{`/`}` never contribute to depth. (b) Bare `.js`/`.ts`/`.mjs`/`.cjs`/`.mts`/`.cts` files previously promoted any `<Identifier` token to a JSX element opener, so a minified IIFE with `r.length<b.length` comparison operators (canonical: `jekyll/lib/jekyll/commands/serve/livereload_assets/livereload.js`) emitted `Unclosed JSX element <b.length>` and dropped the whole file to partial-parse. JSX-mode entry is now gated on file extension: `.jsx`/`.tsx`/`.mdx`/`.astro` stay on; bare JS/TS stays on only when the source carries a leading JSX-import signal (`from "react"` / `require("react")` / `@jsx` pragma in the first 4 KB). Callers that omit `filePath` keep the v0.1.x default (JSX on) so existing tests, rule helpers, and MCP handlers that parse source strings without path context are unchanged. Both paths had been silently dropping rule coverage on real-world codebases — the doctrine from `docs/kb/architecture/ai-first-consumer.md` ("surface, don't suppress") applies at the parser layer too: a partial-parse that swallows a whole file is worse than a false-positive the agent can dismiss in one read.
...
- **MCP `list_rules` echoes filter result shape**: responses now include `matchedOf: { total, matched }` (always present) and `filter: { standard }` (present-when-meaningful, conditional-spread). Previously the handler returned the same `rules: [...]` array whether a filter narrowed the list or was silently a no-op — callers filtering by e.g. `wcag22` (every current rule satisfies a wcag22 criterion) received the full list with no signal that the filter didn't do anything. Per the "verbose meta is scan-confidence signal" rule in `docs/kb/architecture/ai-first-consumer.md`, agents need to distinguish "filter applied and narrowed" from "filter applied and was a no-op" from "no filter" without counting against a known baseline. Additive — the `rules` array shape is unchanged.
...
- **MCP scan responses gain a parallel `warningsDetails` channel** alongside the bare-string `warnings: string[]` ([ADR 0023](./docs/adr/0023-warning-structured-details.md)). Conditional-spread, keyed by `ScanWarningCode`, populated only for codes that carry a structured payload — `extensions_skipped_no_parser` in this release. An agent branching on the warning code can now read `warningsDetails.extensions_skipped_no_parser.{topExtension, topCount, totalSkipped, extensions}` (top five by descending count, ties broken alphabetically) without cross-referencing `meta.analysisCoverage.skippedByExtension` — the gap the bootstrap seventh-pass scan surfaced. The two channels agree on set membership: a code in `warnings[]` without a matching entry is a presence-only signal (`no_config_found`, `scanned_zero_files`, `root_source_defaulted`, `storybook_preset_active`, `scanned_build_artifacts_present`, `session_wrappers_configured_for_different_cwd`, `parse_errors_present`, `tailwind_detected_css_undercounted`, `template_files_parsed_as_literal`, `no_hunks_in_comparison`, `response_token_budget_truncated`); these stay presence-only and leave `warningsDetails` unpopulated. Purely additive — the bare-string channel shape is unchanged, every existing consumer keeps working. Emitted across every scan-family tool that already emits `warnings`: `scan_project`, `scan`, `scan_file`, `scan_diff`, `coverage`, `checklist`, `review_candidates`, `suggest_fix`.
- **`response_token_budget_truncated` warning echoes requested vs. effective file counts** via `warningsDetails.response_token_budget_truncated: { requestedLimit, effectiveLimit }`. Emitted only when the density cap trims trailing file entries on `scan`, `scan_project`, or `scan_diff`. Closes the bootstrap eighth-pass gap: a caller seeing `warnings: ["response_token_budget_truncated"]` + `files.length: 10` previously could not tell aggressive trims (50→10) from marginal ones (50→48) without a re-page. `requestedLimit` is the file count the density guard saw entering (post-pagination, pre-density on `scan_project`; full files-with-findings on `scan` and `scan_diff`, which have no pagination primitive); `effectiveLimit` is what survived the char-budget trim. Conditional-spread per "present-when-meaningful" — omitted when the density cap did not fire. Purely additive; `warnings[]`, `truncated`, and `nextOffset` semantics are unchanged.
- **`conformance_statement` surfaces `limitations[]`** — a top-level prose list, one entry per in-scope criterion whose only signal was absence-of-findings against a runtime-dependent WCAG requirement (keyboard, focus-visible, rendered contrast, heading adequacy, pointer interaction, authentication flow, and the rest of the spec-derived enumeration in `RUNTIME_EVIDENCE_REQUIRED_CRITERIA`). Each entry reads `"<criterionId>: <title> — runtime evidence required; no static finding in scope, no attestation supplied."`. The same criteria also surface in `blockers[]` with `reason: "runtime-evidence-required"` and `status: "undetermined"` so agents can route by structured type or by prose. Present-when-non-empty per CLAUDE.md §1 "present-when-meaningful"; a statement with no runtime-dependent SC in scope omits the field entirely rather than emitting `limitations: []`. Markdown renderer emits a `## Limitations` section in the same cases. Closes the silent-miss case the AI-first consumer doctrine exists to prevent — a bootstrap scan returning `status: "pass", reason: "no-evidence"` for 2.1.1 / 2.4.3 / 2.4.7 / 1.4.3 / 1.4.11 / 2.4.6 read to a claim consumer as "checked clean" when the reality was "the static layer never had the axis to answer."

### Changed

- **MCP `checklist` inverts the default shape of `untargetedCriteriaList`** (`src/mcp/tool-checklist.ts`). Previously the full list was gated behind `showUntargeted: true` while the summary reported only a count — per `docs/kb/architecture/ai-first-consumer.md` ("surface, don't suppress"), agents preparing a VPAT or running a formal audit needed to make a second call to enumerate the bare criterion IDs. The default now ships `untargetedCriteriaList` as a bare criterion-ID array so the enumeration is cheap. `showUntargeted: true` still opts into the full-item shape (`title` + `level` + `principle` + empty `candidates`) for workflows that need the metadata; `showUntargeted: false` is preserved as a size-pressure escape hatch that omits the list entirely while the summary counter remains visible. Closes V1-UNTARGETED-CRITERIA-DEFAULT-EMIT.
- **MCP `propose_baseline` hoists per-entry rationale prose into a response-level `rationales` map** (`src/mcp/propose-baseline-classify.ts`, `src/mcp/tool-propose-baseline.ts`). Entries previously carried `rationale: string` inline; on a 242-finding scan the same 128-char `unclassified` rationale shipped 242 times, inflating the response past 70 KB on one line (V1-PROPOSE-BASELINE-RATIONALE-DEDUP; reproduced at 10.3 MB / 33,386 entries on website-templates and 564 KB / 1,818 entries on Bootstrap). Each entry now carries `rationaleKey: string` — a 12-hex SHA-256 truncation that points into a top-level `rationales: { [rationaleKey]: string }` map. Identical rationales across entries collapse to one key; specific-evidence rationales (`legacy-route` / `design-system-internal` / `third-party-html` / `wrapper-undetected`, which embed matched paths or wrapper names) dedup when two entries happen to produce the same text. Shape mirrors the existing `referenceGuide.fixDescriptions[ruleId][hash]` hoist so agents recognize the short-hex-token format across the MCP surface. Consumers that read `entry.rationale` now read `response.rationales[entry.rationaleKey]`; the mapping is a one-line resolve. Counts and `proposed.length` are unchanged — the hoist is pure prose dedup, no finding is dropped or downgraded (surface-don't-suppress still holds).
- **MCP scan `meta.rulesEvaluated` and `meta.perRuleCoverage` now agree by construction** (V1-META-RULES-EVALUATED-COVERAGE-DRIFT). Previously the counter and the per-rule array could drift — project-scoped rules (`focus/outline-visible`, `wrapper/drift`, whose lifecycle is `afterProject` only) never flowed through the per-file tracker and were silently omitted from `perRuleCoverage`, while `rulesEvaluated` counted them. An agent reading both surfaces got a pair it couldn't reconcile and, worse, couldn't tell "ran-with-zero-eligible-files" from "never-ran" for the missing rules. Every evaluated rule now gets exactly one entry: project-scoped rules surface as `filesEvaluated: filesScanned, coverageConfidence: "high"` (treating the full file set as one evaluation); extension-gated rules with zero eligible files keep the existing `filesEvaluated: 0, coverageConfidence: "low"` shape with a `reason` naming the gap. `rulesEvaluated` is now derived from `perRuleCoverage.length` so the two cannot drift again. Agents that read `meta.perRuleCoverage` see more rows on typical scans — previously absent project-scoped rows now appear.
...
- **MCP `scan_project` default `limit` lowered from 200 to 25** ([ADR 0021](./docs/adr/0021-scan-response-size-budget.md)). The previous default shipped responses up to ~1.2 MB on medium repos and tripped typical MCP host token ceilings on the first call even at `limit: 50`. Bytes-per-file is stable within a given repo profile (~2.9 KB/file amortized on a Bootstrap-class scan), so a tighter file-count cap is the cheapest honest fix; the token-aware-budget alternative was considered and deferred. Pagination semantics (`truncated`, `nextOffset`, `totalFilesWithFindings`) are unchanged — callers that want the old window set `limit: 200` explicitly; the 2000-file maximum is also unchanged. Consumers iterating via `nextOffset` continue to work without modification.
- **MCP scan responses gain a token-density secondary budget** ([ADR 0021 amendment](./docs/adr/0021-scan-response-size-budget.md#amendment-2026-04-20-token-density-secondary-budget)). After the `fix.description` hoist runs, `scan` / `scan_project` / `scan_diff` measure the serialized response; if density pushes it past the 88000-char default (≈ 22000 tokens via a 4-chars-per-token proxy, leaving headroom under the ~25k-token MCP host ceiling), trailing file entries drop until the body fits. Truncated responses flip `truncated: true`, surface `nextOffset = offset + keptFileCount` (where paging applies), and emit a new `response_token_budget_truncated` warning code so callers can distinguish density truncation from the existing file-count cap. Progress guarantee: at least one file entry is always kept so the caller has resumable state. The secondary cap addresses the Bootstrap-class failure mode the original ADR flagged as a follow-up — 18 files / 131 findings / ~107 KB that the file-count cap alone could not bound because it never engaged. `scan_file` is unaffected (single-file shape; no trailing file entries to drop).
...
- **MCP `meta.analysisCoverage` emits canonical `rulesEligibleByExtension`; both pre-release predecessors (`rulesByExtension`, `rulesFiredByExtension`) dropped before any tagged release** (`src/mcp/analysis-coverage.ts`, `src/mcp/warnings.ts`, [ADR 0028](./docs/adr/0028-rules-by-extension-rename.md)). Field-test sweeps observed the prior `rulesFiredByExtension` name failing the same doctrine bar ADR 0028 was originally opened to enforce: on a fragment-only `.md` README scan, `rulesFiredByExtension['.md']` listed 96+ rules — including `media/audio-video-no-controls`, `semantics/landmark-main`, `parsing/duplicate-id` — when zero rules actually emitted output. The "fired" verb is a deterministic-sounding token that lies because the values measure extension-gate eligibility (every active rule whose `appliesTo.fileExtensions` matches the extension via `extensionMatches`, plus every rule with no extension constraint), not whether `check()` produced anything. This is the canonical "Heuristic-mislabeled meta sub-fields are dishonest" failure mode in [`docs/kb/architecture/ai-first-consumer.md`](./docs/kb/architecture/ai-first-consumer.md) — applied at the field-name level rather than at a sub-field token. The field is renamed in-place to `rulesEligibleByExtension` so the name matches what the values measure; the per-rule actual-fire surface remains `meta.perRuleCoverage[].filesEvaluated`. Per the same precedent that dropped the `rulesByExtension` alias one entry up the version history, no transition alias and no narrating deprecation warning ride alongside — the dual-emission failure mode would recur identically. No tagged release ever exposed either pre-release name (`rulesByExtension` or `rulesFiredByExtension`), so the rename is non-breaking. Original (now superseded) entry below preserved unchanged in the changelog history.

- **MCP `meta.analysisCoverage` emits canonical `rulesFiredByExtension` only; legacy duplicate `rulesByExtension` alias dropped before any tagged release** (`src/mcp/analysis-coverage.ts`, `src/mcp/warnings.ts`, [ADR 0028](./docs/adr/0028-rules-by-extension-rename.md)). The per-extension eligibility view and the per-rule `meta.perRuleCoverage` tally previously shared a look-alike "what ran?" shape under different field names — `rulesByExtension[ext]` listed rules whose extension gate matched files of `ext`, while `perRuleCoverage[].filesEvaluated` carried the post-runner per-rule count. On a `scan_file` of `accessibility.mdx` the first returned ~2 entries (extension-gate match) while the second returned ~53 (every rule the runner attempted), and agents joining the two surfaces silently disagreed on the answer (V1-RULES-BY-EXTENSION-LABELING). The canonical name is now `rulesFiredByExtension` — semantically distinct from the per-rule tally so the two surfaces no longer compete for the same role. An earlier `[Unreleased]` iteration shipped the canonical name alongside a duplicate `rulesByExtension` alias plus a `deprecated_field_rules_by_extension_renamed_rules_fired_by_extension` warning code intended to narrate the migration window. Both fields carried identical values on every coverage response across every verbose `meta` payload — exactly the canonical "Ambiguous field shapes are dishonest" failure mode in [`docs/kb/architecture/ai-first-consumer.md`](./docs/kb/architecture/ai-first-consumer.md) (recurring across corpora, inflating bulk payloads ~6–10 KB). The duplicate `rulesByExtension` field and its narrating warning code (`deprecated_field_rules_by_extension_renamed_rules_fired_by_extension` + the `warningsDetails` marker) are removed before any release tagged the alias, so no shipped consumer ever read `rulesByExtension`. Consumers read `rulesFiredByExtension`. Closes Q9-RULESBYEXTENSION-DUP-PAYLOAD (and the original V1-RULES-BY-EXTENSION-LABELING closure stays — the canonical-field rename was correct; only the dual-emission shape was wrong).
- **MCP `coverage` entry arrays emit canonical `criterionId` only; legacy duplicate `id` field dropped before any tagged release** (`src/mcp/tool-coverage.ts`, `src/mcp/warnings.ts`). The criterion-bearing arrays on every coverage entry — `failingAutomatedCriteria`, `warningAutomatedCriteria`, `manualWithCandidates`, `likelyIrrelevantCriteria`, `untestableCriteria`, and `untargetedCriteriaList` (when shown) — historically named the criterion field `id`, while the sibling `checklist.items[].criterionId` already used the canonical name. The earlier `[Unreleased]` entry shipped the canonical `criterionId` alongside a duplicate `id` alias plus a `deprecated_field_id_renamed_criterionId` warning code intended to narrate the migration window. Both fields carried identical values on every entry across every coverage response — exactly the canonical "Ambiguous field shapes are dishonest" failure mode in [`docs/kb/architecture/ai-first-consumer.md`](./docs/kb/architecture/ai-first-consumer.md) (recurring across corpora, inflating bulk payloads ~6 KB). The duplicate `id` field and its narrating warning code (`deprecated_field_id_renamed_criterionId` + the `warningsDetails` marker) are removed before any release tagged the alias, so no shipped consumer ever read `id`. Consumers join the two surfaces by `criterionId`. Closes Q9-DUPLICATE-ID-AND-CRITERIONID-AFTER-RENAME (and the original Q7-CRITERION-ID-FIELD-NAME-DRIFT closure stays — the canonical-field rename was correct; only the dual-emission shape was wrong).
- **Rule `navigation/href-placeholder` split into `navigation/href-javascript-scheme` and `navigation/href-empty-fragment`** (`src/rules/navigation/href-javascript-scheme.ts`, `src/rules/navigation/href-empty-fragment.ts`). The umbrella `href-placeholder` rule (itself the V1-RULE-NAVIGATION-HREF-VOID-RENAME hop from `href-javascript-void`) covered three distinct placeholder shapes — `javascript:` schemes, bare `href="#"`, and empty `href=""` — under one ID. A pragma or config entry suppressing the umbrella ID silenced ALL three shapes; an agent triaging by ID could not distinguish "we know this jQuery toggle uses `javascript:void`" from "we know this Forgot-password? link is intentionally a placeholder we'll wire up later." The split moves each shape to its own rule ID with its own message and suggestion, so suppression and triage are granular per-shape. Both legacy IDs (`navigation/href-javascript-void` and `navigation/href-placeholder`) keep resolving through the rule-ID alias table (`src/engine/rule-aliases.ts`) — both point at `navigation/href-javascript-scheme` because the original `href-javascript-void` literal name unambiguously names that shape and pointing the umbrella legacy ID at the empty-fragment heir would silently flip suppression intent for the most common legacy callers. Every alias resolution emits the structured `deprecated_rule_id:<from>:navigation/href-javascript-scheme` warning so agents can offer to rewrite (and consider whether `href-empty-fragment` also needs explicit suppression — the second new ID stays live for the previously-too-broad umbrella callers, which IS the surface-don't-suppress doctrine answer per `docs/kb/architecture/ai-first-consumer.md`). Deprecated since 0.2.0; aliases dropped in 0.3.0. Ships as minor, not breaking, per CLAUDE.md §12. Closes Q7-NAVIGATION-HREF-RULE-RENAME.
- **Rule `navigation/href-javascript-void` renamed to `navigation/href-placeholder`** (`src/rules/navigation/href-placeholder.ts`). The old name promised detection of `href="javascript:void(0)"` only, but the check always also fired on bare `href="#"`, whitespace-only, and `href=""` — every shape of placeholder href. The new name matches the actual surface. The old ID keeps resolving through the rule-ID alias table (`src/engine/rule-aliases.ts`, V1-INFRA-RULE-ID-ALIAS-TABLE) in suppression pragmas, `ra11y.config.ts` `rules` keys, CLI `--rules` flags, and MCP lookups — every resolution emits the structured `deprecated_rule_id:navigation/href-javascript-void:navigation/href-placeholder` warning so agents can offer to rewrite. `list_rules` surfaces the old ID with `deprecated: true` + `replacedBy: "navigation/href-placeholder"`. Deprecated since 0.2.0; alias dropped in 0.3.0 — once the alias is removed the old ID stops being accepted, which is when the rename becomes a hard break. Ships as minor, not breaking, per CLAUDE.md §12. Closes V1-RULE-NAVIGATION-HREF-VOID-RENAME.
