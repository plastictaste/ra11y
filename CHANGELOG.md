...
- **MCP `scan_project` surfaces `truncated_files_dropped` warning with rule-level impact when truncation drops files-with-findings** (`src/mcp/warnings.ts` `truncatedFilesDroppedDetailsField`, `src/mcp/truncated-files-dropped.ts`, wired in `src/mcp/scan-project-budget.ts` for both the density-cap path and the slim-envelope fallback). The byte-level codes (`response_token_budget_truncated`, `response_dropped_files_oversize`) name how MUCH of the response was clipped; the new code names WHICH rule families' findings just disappeared from the wire so the agent can decide between recoveries (a single dominant rule's noise was clipped → narrow via `restrictToPaths` to that rule's families; the cross-rule signal was uniformly clipped → just widen `limit`). Paired payload: `warningsDetails.truncated_files_dropped: { droppedFileCount, ruleFamiliesAffected, topDroppedRules }` where `ruleFamiliesAffected` is the unique set of rule prefixes (e.g. `keyboard`, `aria`) sorted alphabetically and `topDroppedRules` is the top 10 `(ruleId, droppedCount)` entries sorted by count desc with alphabetical tie-break. Per [`docs/kb/architecture/ai-first-consumer.md`](./docs/kb/architecture/ai-first-consumer.md) "Oversize-success is ambiguous failure" applied to the file-level analogue: an agent reading the byte-level warnings alone cannot tell which rule families' findings just disappeared from this response; the rule-level payload closes the silent-miss gap. Cross-surface: response-instance code (belongs to the originating tool only); does NOT propagate to `checklist` / `coverage`. Purely additive.
