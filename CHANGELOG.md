...
- **MCP `meta.analysisCoverage.rulesByExtension` renamed to `rulesFiredByExtension`; legacy name remains for one release as a transition alias** (`src/mcp/analysis-coverage.ts`, [ADR 0028](./docs/adr/0028-rules-by-extension-rename.md)). The per-extension eligibility view and the per-rule `meta.perRuleCoverage` tally previously shared a look-alike "what ran?" shape under different field names — `rulesByExtension[ext]` listed rules whose extension gate matched files of `ext`, while `perRuleCoverage[].filesEvaluated` carried the post-runner per-rule count. On a `scan_file` of `accessibility.mdx` the first returned ~2 entries (extension-gate match) while the second returned ~53 (every rule the runner attempted), and agents joining the two surfaces silently disagreed on the answer (V1-RULES-BY-EXTENSION-LABELING). The canonical name is now `rulesFiredByExtension` — semantically distinct from the per-rule tally so the two surfaces no longer compete for the same role. The legacy `rulesByExtension` field still ships alongside `rulesFiredByExtension` for one minor release with the identical value; a new structured `warnings: ["deprecated_field_rules_by_extension_renamed_rules_fired_by_extension"]` code fires whenever the alias rides so callers can drop their `rulesByExtension` reads on the next call. The alias is removed in the next minor release. Mirror precedent: `deprecated_field_id_renamed_criterionId` (Q7-CRITERION-ID-FIELD-NAME-DRIFT) — same alias-plus-warning pattern. Closes V1-RULES-BY-EXTENSION-LABELING.
- **MCP `coverage` entry arrays now emit canonical `criterionId`; legacy `id` remains for one release as a transition alias** (`src/mcp/tool-coverage.ts`). The criterion-bearing arrays on every coverage entry — `failingAutomatedCriteria`, `manualWithCandidates`, `likelyIrrelevantCriteria`, `untestableCriteria`, and `untargetedCriteriaList` (when shown) — historically named the criterion field `id`, while the sibling `checklist.items[].criterionId` already used the canonical name. An agent joining the two surfaces by criterion silently mis-matched on field name, exactly the cross-surface drift `docs/kb/architecture/ai-first-consumer.md` ("One tool call should answer 'what next?'") warns against. Each entry now carries both `criterionId` (canonical, matches the namespaced-id convention used elsewhere — `wcag22:1.4.3`) and `id` (deprecated alias, identical value) so callers reading either name keep working. A new structured `warnings: ["deprecated_field_id_renamed_criterionId"]` code fires on every `coverage` response while the alias is shipped so agents can drop their `id` reads on the next call. The `id` alias will be removed in the next minor release; this CHANGELOG entry tracks the removal window. Closes Q7-CRITERION-ID-FIELD-NAME-DRIFT.
