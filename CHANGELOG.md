...
- **MCP `bootstrap` response `scan` subset drops `totalFindings`; adds `violationsCount` + `notesCount` + `mechanicalEditsAvailable` + `fixesByClass`** (`src/mcp/tool-bootstrap.ts` `ScanSubset`). Upstream `scan_project` intentionally splits its plan block into `violations` / `notes` / `mechanicalEditsAvailable` / `fixesByClass` (one counter per remediation lane); the bootstrap composer used to re-sum them back into a single `scan.totalFindings` headline, which re-introduced the composite-headline anti-pattern CLAUDE.md §1 warns against — agents budgeting against the sum treated info-severity notes as identical work to violations, and could not tell mechanical-lane violations apart from guidance/runtime-only/verify-in-source lanes without a second tool call. The subset now forwards the upstream split verbatim: `violationsCount` (severity `error` / `warning`), `notesCount` (severity `info`), `mechanicalEditsAvailable` (violations with inline `fixPaths.primary.edit`; present-when-meaningful), and `fixesByClass: { mechanical, guidance, runtimeOnly, verifyInSource }` (present when violations > 0, mirroring the upstream `emitFixesByClass` gate). The `nextStep` prose reads "N violations" (and optionally "+ M notes") instead of "N findings"; `nextStepStructured` routes off `violationsCount` rather than the composite. Consumers that read `response.scan.totalFindings` read `response.scan.violationsCount + response.scan.notesCount` for the former sum, or branch on `violationsCount` alone for the work-budget question `nextStepStructured` answers.
