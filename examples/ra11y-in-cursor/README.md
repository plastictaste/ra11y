...
- **Respect the split counters.** `plan.mechanicalEditsAvailable` and `plan.guidanceFixesAvailable` are separate on purpose — they describe categorically different work. Never sum them into a headline "fixes available" number.
