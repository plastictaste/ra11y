...
- **Respect the split counters.** `plan.fixesByClass.mechanical`, `.guidance`, `.runtimeOnly`, and `.verifyInSource` are separate on purpose — each describes categorically different remediation work. Never sum all four into a headline "fixes available" number; the agent's budget should reflect which lane the work actually falls in. For the "apply-fix can batch this" subset specifically, sum only the two editable lanes: `fixesByClass.mechanical + fixesByClass.verifyInSource`.
