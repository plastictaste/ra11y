...
import { posixJoin, toPosix } from "../../helpers/path.ts";

function mkTmp(): string {
  return mkdtempSync(posixJoin(tmpdir(), "ra11y-discover-"));
}

/** Marks a directory as a git repo root without actually initializing git. */
function markGitRoot(dir: string): void {
  mkdirSync(posixJoin(dir, ".git"), { recursive: true });
  // A bare `.git/HEAD` is enough for us — our detector just stats `.git`.
  writeFileSync(posixJoin(dir, ".git", "HEAD"), "ref: refs/heads/main\n");
}
...
describe("discoverFiles .gitignore walk-up", () => {
  ...
  it("subpath scan honors root .gitignore", async () => {
    ...
    markGitRoot(dir);
    write(posixJoin(dir, ".gitignore"), "dist/\n");
    write(posixJoin(dir, "src", "app", "page.tsx"));
    write(posixJoin(dir, "src", "app", "dist", "gen.tsx"));
    write(posixJoin(dir, "src", "app", "dist", "nested", "more.tsx"));

    const found = await discoverFiles([posixJoin(dir, "src", "app")]);
    const rel = found.map((p) => p.slice(toPosix(dir).length + 1));
  ...
  it("full-repo scan is unchanged when cwd === gitRoot", async () => {
    ...
    write(posixJoin(dir, ".gitignore"), "dist/\n");
    write(posixJoin(dir, "src", "app", "page.tsx"));
    write(posixJoin(dir, "dist", "bundle.tsx"));

    const fromRoot = await discoverFiles([dir]);
    const relFromRoot = fromRoot.map((p) => p.slice(toPosix(dir).length + 1)).sort();
    ...
    const fromSubpath = await discoverFiles([posixJoin(dir, "src", "app")]);
    const relFromSubpath = fromSubpath.map((p) => p.slice(toPosix(dir).length + 1)).sort();
  ...
  it("non-git directory: walk-up does nothing, existing behavior intact", async () => {
    ...
    write(posixJoin(dir, "src", "app", "page.tsx"));
    write(posixJoin(dir, "src", "app", "dist", "bundle.tsx"));

    const found = await discoverFiles([posixJoin(dir, "src", "app")]);
    const rel = found.map((p) => p.slice(toPosix(dir).length + 1)).sort();
  ...
  it("composes multiple intervening .gitignore files correctly", async () => {
    ...
    markGitRoot(dir);
    write(posixJoin(dir, ".gitignore"), "logs/\n");
    write(posixJoin(dir, "packages", ".gitignore"), "build/\n");
    const scanRoot = posixJoin(dir, "packages", "web", "src");
    write(posixJoin(scanRoot, "page.tsx"));
    write(posixJoin(scanRoot, "app", "app.tsx"));
    write(posixJoin(scanRoot, "build", "gen.tsx"));
    write(posixJoin(scanRoot, "logs", "trace.tsx"));
  ...
  it("anchored ancestor patterns outside the scan root are discarded", async () => {
    ...
    markGitRoot(dir);
    write(posixJoin(dir, ".gitignore"), "/other-pkg/\n");
    const scanRoot = posixJoin(dir, "web");
    write(posixJoin(scanRoot, "page.tsx"));
    write(posixJoin(scanRoot, "other-pkg", "file.tsx"));
  ...
  it("anchored ancestor patterns inside the scan root are re-anchored", async () => {
    ...
    markGitRoot(dir);
    write(posixJoin(dir, ".gitignore"), "/web/generated/\n");
    const scanRoot = posixJoin(dir, "web");
    write(posixJoin(scanRoot, "page.tsx"));
    write(posixJoin(scanRoot, "generated", "bundle.tsx"));
    write(posixJoin(scanRoot, "nested", "generated", "side.tsx"));
  ...
  it("unanchored root pattern still matches when scanning a subpath", async () => {
    ...
    markGitRoot(dir);
    write(posixJoin(dir, ".gitignore"), "custom-dir\n");
    const scanRoot = posixJoin(dir, "web");
    write(posixJoin(scanRoot, "page.tsx"));
    write(posixJoin(scanRoot, "custom-dir", "bundle.tsx"));
    write(posixJoin(scanRoot, "nested", "custom-dir", "bundle.tsx"));
  ...
  it("respectGitignore:false skips the walk-up entirely", async () => {
    markGitRoot(dir);
    write(posixJoin(dir, ".gitignore"), "dist/\n");
    const scanRoot = posixJoin(dir, "src", "app");
    write(posixJoin(scanRoot, "page.tsx"));
    write(posixJoin(scanRoot, "dist", "gen.tsx"));
...
describe("discoverFilesWithDiagnostics", () => {
  ...
  it("counts files skipped because their extension isn't parseable", async () => {
    ...
    write(posixJoin(dir, "page.tsx"));
    write(posixJoin(dir, "Button.svelte"));
    write(posixJoin(dir, "app.vue"));
    write(posixJoin(dir, "Layout.svelte"));
    write(posixJoin(dir, "README"));

    const result = await discoverFilesWithDiagnostics([dir]);
    const rel = result.files.map((p) => p.slice(toPosix(dir).length + 1)).sort();
  ...
  it("splits well-known textual no-extension filenames into named buckets", async () => {
    ...
    write(posixJoin(dir, "page.tsx"));
    write(posixJoin(dir, "LICENSE"));
    write(posixJoin(dir, "Makefile"));
    write(posixJoin(dir, "Dockerfile"));
    write(posixJoin(dir, "NOTICE"));
    write(posixJoin(dir, "Rakefile"));
    write(posixJoin(dir, "weird-blob")); // residual no-ext bucket
  ...
  it("matches well-known textual no-extension filenames case-insensitively (canonical case on the wire)", async () => {
    ...
    const sub = posixJoin(dir, "lower");
    write(posixJoin(sub, "license"));
    write(posixJoin(dir, "page.tsx"));
  ...
  it("returns an empty skip map for clean all-parseable directories", async () => {
    write(posixJoin(dir, "page.tsx"));
    write(posixJoin(dir, "styles.css"));
    write(posixJoin(dir, "page.html"));
  ...
  it("does not count files rejected by default excludes or gitignore", async () => {
    ...
    // suppression.
    markGitRoot(dir);
    write(posixJoin(dir, ".gitignore"), "bundle.tsx\n");
    write(posixJoin(dir, "page.tsx"));
    write(posixJoin(dir, "__mocks__", "fs.ts"));
    write(posixJoin(dir, "bundle.tsx"));
    // Still count a true extension skip so we know the filter
    // distinguishes the two paths.
    write(posixJoin(dir, "Card.svelte"));
  ...
  it("counts parseable files filtered by gitignore under excludedByPatternByExtension", async () => {
    ...
    markGitRoot(dir);
    write(posixJoin(dir, ".gitignore"), "generated/\n");
    write(posixJoin(dir, "src", "page.css"));
    write(posixJoin(dir, "src", "app.js"));
    write(posixJoin(dir, "generated", "compiled.css"));
    write(posixJoin(dir, "generated", "compiled.js"));
    write(posixJoin(dir, "generated", "deeper", "extra.css"));
  ...
  it("counts user-excluded parseable files under excludedByPatternByExtension", async () => {
    ...
    write(posixJoin(dir, "src", "page.tsx"));
    write(posixJoin(dir, "third-party", "third.tsx"));
    write(posixJoin(dir, "third-party", "third.css"));
    write(posixJoin(dir, "src", "page.css"));
  ...
  it("closes the per-extension accounting invariant on a corpus with mixed drop axes", async () => {
    ...
    write(posixJoin(dir, ".gitignore"), "generated/\n");
    write(posixJoin(dir, "src", "page.css"));
    write(posixJoin(dir, "src", "app.tsx"));
    write(posixJoin(dir, "src", "note.txt"));
    write(posixJoin(dir, "generated", "compiled.css"));
    write(posixJoin(dir, "generated", "big.tsx"));
    write(posixJoin(dir, "__mocks__", "fs.ts"));
    write(posixJoin(dir, "third-party", "lib.tsx"));
    write(posixJoin(dir, "assets", "app.css.map"));
  ...
  it("includes test/spec/story/dev-tools files in discovery by default", async () => {
    ...
    write(posixJoin(dir, "page.tsx"));
    write(posixJoin(dir, "page.test.tsx"));
    write(posixJoin(dir, "page.spec.ts"));
    write(posixJoin(dir, "page.stories.tsx"));
    write(posixJoin(dir, "page.story.jsx"));
    write(posixJoin(dir, "__tests__", "Form.tsx"));
    write(posixJoin(dir, "stories", "Button.tsx"));
    write(posixJoin(dir, "dev-tools", "panel.tsx"));
    write(posixJoin(dir, "devtools", "inspector.tsx"));
  ...
  it("includeTests:true drops the __mocks__ default exclusion too", async () => {
    ...
    const defaultRel = defaultRun.map((p) => p.slice(toPosix(dir).length + 1)).sort();
    expect(defaultRel).toEqual(["page.tsx"]);

    const includeRun = await discoverFiles([dir], { includeTests: true });
    const includeRel = includeRun.map((p) => p.slice(toPosix(dir).length + 1)).sort();
  ...
  it("routes `.map` sourcemap files into a dedicated bucket rather than skippedByExtension", async () => {
    ...
    write(posixJoin(dir, "page.tsx"));
    write(posixJoin(dir, "assets/app.css.map"));
    write(posixJoin(dir, "assets/vendor.js.map"));
    write(posixJoin(dir, "Card.svelte"));
  ...
  it("returns sourcemapFiles ascending lexically so the wire shape is deterministic", async () => {
    ...
    write(posixJoin(dir, "page.tsx"));
    write(posixJoin(dir, "z.css.map"));
    write(posixJoin(dir, "a.css.map"));
    write(posixJoin(dir, "m.css.map"));
  ...
  it("surfaces every default-excluded build-artifact directory that contained a parseable file", async () => {
    ...
    write(posixJoin(dir, "src", "page.tsx"));
    write(posixJoin(dir, "dist", "bundle.js"));
    write(posixJoin(dir, "dist", "page.html"));
    write(posixJoin(dir, ".next", "static", "chunk.js"));
    // node_modules is in DEFAULT_IGNORED_DIRS but is intentionally
    // NOT in the surfaced subset — agents already expect it dropped.
    write(posixJoin(dir, "node_modules", "lib", "index.js"));
  ...
  it("drops default-excluded directories that contain no parseable files", async () => {
    ...
    // extension.
    write(posixJoin(dir, "src", "page.tsx"));
    write(posixJoin(dir, "dist", "logo.png"));
    write(posixJoin(dir, "build", "bundle.js.map"));
  ...
  it("returns defaultExcludedArtifactPaths sorted by directory path", async () => {
    write(posixJoin(dir, "src", "page.tsx"));
    write(posixJoin(dir, "out", "bundle.js"));
    write(posixJoin(dir, "build", "bundle.js"));
    write(posixJoin(dir, "dist", "bundle.js"));
