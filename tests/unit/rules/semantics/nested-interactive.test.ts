...
describe("rule semantics/nested-interactive", () => {
  ...
  describe("outer element must be materially interactive", () => {
    // ARIA role on the outer element alone does NOT qualify it as a
    // nested-interactive outer. A `<div role="tab">` or
    // `<li role="menuitem">` with no click/key handler, no tabindex,
    // and no native interactive tag is a semantic wrapper — the inner
    // `<a href>` or `<button>` is the sole focus + activation surface,
    // so there is no two-focusables-one-control problem to surface.
    // The outer-ancestor predicate requires one of: native interactive
    // tag, tabindex>=0, or an inline on* handler.

    it("HTML: <a href> inside <div role=tab> does NOT fire (role-only wrapper, no handler, no tabindex)", () => {
    ...
    it("HTML: <button> inside <li role=menuitem> does NOT fire (role-only wrapper)", () => {
    ...
    it("HTML: <a href> inside <div role=tab tabindex=0> DOES fire (tabindex makes outer materially interactive)", () => {
      const v = runRule(rule, `<div role="tab" tabindex="0"><a href="#panel">Section 1</a></div>`, {
        filePath: "a.html",
      });
    ...
    it("HTML: <a href> inside <div role=button onclick> DOES fire (handler makes outer materially interactive)", () => {
      const v = runRule(rule, `<div role="button" onclick="activate()"><a href="/x">Go</a></div>`, {
        filePath: "a.html",
      });
      expect(v).toHaveLength(1);
      expect(v[0]?.message).toContain(`role="button"`);
    });

    it("HTML: <a href> inside <div role=tab tabindex=-1> does NOT fire (tabindex=-1 is not a focus stop)", () => {
      const v = runRule(
        rule,
        `<div role="tab" tabindex="-1"><a href="#panel">Section 1</a></div>`,
        { filePath: "a.html" },
      );
      expect(v).toHaveLength(0);
    });

    it("JSX: <button> inside <li role=menuitem> does NOT fire (role-only wrapper)", () => {
      const v = runRule(rule, `const X = <li role="menuitem"><button>Open</button></li>;`);
      expect(v).toHaveLength(0);
    });

    it("JSX: <button> inside <div role=button onClick> DOES fire (handler makes outer materially interactive)", () => {
      const v = runRule(
        rule,
        `const X = <div role="button" onClick={activate}><button>Open</button></div>;`,
      );
      expect(v).toHaveLength(1);
      expect(v[0]?.message).toContain(`role="button"`);
    });

    it("JSX: <a href> inside <div tabIndex={0} role=tab> DOES fire (tabIndex makes outer materially interactive)", () => {
      const v = runRule(rule, `const X = <div role="tab" tabIndex={0}><a href="/x">Go</a></div>;`);
      expect(v).toHaveLength(1);
      expect(v[0]?.message).toContain(`role="tab"`);
    });
  });
  ...
  describe("composite-widget role enrichment (ARIA 1.2)", () => {
    // The composite-widget note is additive framing applied when the
    // outer element is materially interactive AND also carries a
    // delegating role (tab / menuitem / option / treeitem / gridcell /
    // row). After the Q3 tightening, role-alone no longer qualifies
    // the outer element, so these tests pair the role with a native
    // interactive tag, an inline handler, or a focusable tabindex.

    it("HTML: <a href> inside <button role=menuitem> fires with composite-widget note", () => {
      const v = runRule(rule, `<button role="menuitem">Go <a href="/x">link</a></button>`, {
        filePath: "a.html",
      });
      expect(v).toHaveLength(1);
      expect(v[0]?.severity).toBe("error");
    ...
    it("HTML: <a href> inside <div role=tab tabindex=0> fires with composite-widget note", () => {
      const v = runRule(rule, `<div role="tab" tabindex="0"><a href="/x">Tab</a></div>`, {
        filePath: "a.html",
      });
      expect(v).toHaveLength(1);
      expect(v[0]?.message).toContain(`role="tab"`);
    ...
    it("HTML: <a href> inside <button> fires WITHOUT composite-widget note (no delegating role)", () => {
    ...
    it("JSX: <a href> inside <button role=menuitem> fires with composite-widget note", () => {
      const v = runRule(
        rule,
        `const X = <button role="menuitem">Go <a href="/x">link</a></button>;`,
      );
      expect(v).toHaveLength(1);
      expect(v[0]?.message).toContain(`role="menuitem"`);
      expect(v[0]?.message).toContain("composite-widget");
