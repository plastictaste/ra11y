...
/**
 * elides the
 * contents of triple-backtick fenced code blocks, indented (4-space /
 * tab) code blocks, and single-backtick inline-code spans from a
 * markdown source so downstream template-directive detection doesn't
 * fire on prose examples that QUOTE template syntax (a Jekyll docs
 * page showing `<%= Time.now %>` as an ERB example, a release-note
 * embedding `{% assign %}`).
 *
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (fenceLen === 0) {
      ...
      if (canStartIndentedBlock && INDENTED_CODE_RE.test(line)) {
        // Walk forward eliding the run; CommonMark §4.4 lets blank
        // lines sit between indented code lines without breaking the
        // block, so long as the next non-blank line is also indented.
        lines[i] = "";
        let j = i + 1;
        while (j < lines.length) {
          ...
          if (INDENTED_CODE_RE.test(next)) {
            // Erase every blank-or-indented line we walked past.
            for (let k = i + 1; k <= j; k++) {
              const inner = lines[k] ?? "";
