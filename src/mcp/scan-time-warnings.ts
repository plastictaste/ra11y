// ra11y-limits-exempt: shared scan-time aggregator threads each project-rooted MCP tool's scan-confidence telemetry through a single seam; the paired-warnings trim summary additions push it over 500 effective lines, but extracting sections would fragment the cross-surface contract this file documents and enforces. Future refactor: split per warning code family.
...
function deriveBuildArtifactSignals(inputs: ScanTimeWarningInputs): DerivedBuildArtifactSignals {
  const aux = inputs.auxiliarySiblingPaths;
  const buildArtifactEntries = collectBuildArtifacts(inputs.parsedFiles, aux);
  ...
  const scannedMinifiedFiles = buildArtifactEntries
    ...
    .map((e) => e.path);
  // Trim minified subset to `{ count, topPath, top: [10] }` — same
  // envelope shape `scanned_build_artifacts_present` ships (see
  // `scannedBuildArtifactsSummary` block below for the parallel
  // inline trim). Full identity rides on `meta.scannedBuildArtifacts`.
...
/**
 * Builds the spreadable minified-files subset of {@link WarningInputs}.
 * Both inputs ride together: the predicate-gating list fires the
 * bare warning code, and the summary drives the
 * `warningsDetails.scanned_minified_file` payload. Empty list omits
 * both; non-empty threads them through together. Extracted from
 * {@link buildWarningsFieldInputs} so the orchestrator stays under
 * the cognitive-complexity cap.
 */
function scannedMinifiedInputs(derived: DerivedBuildArtifactSignals): Partial<WarningInputs> {
  if (derived.scannedMinifiedFiles.length === 0) return {};
  return {
