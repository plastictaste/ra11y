 ...
 * only the frontmatter `<script>` island (component-script blanked by
 * {@link parseAstro}) and the residual HTML template with imported
 * components left as opaque elements and `{expr}` braces passed
 * through as literal text. The rendered output an Astro server-render
 * would produce — `<main>` injected by a `<Layout>` import, page
 * `<title>` set by an SEO component, `lang=` on the framework root,
 * `aria-*` attributes the component spreads, listener wiring the
 * island runtime attaches — is invisible to the scanner. Per the
 * AI-first doctrine "Parser-failure invalidates per-file confidence"
 * (extended to template-island parse-as-literal classifications), any
 * per-rule entry crediting an `astro_islands_unrendered` file must
 * downgrade so the per-rule label reflects the substrate.
 *
 * The set includes (a) the document-shaped rules whose envelope is
 * supplied by the unrendered Astro layout — already in
 * {@link FRAGMENT_DOWNGRADE_RULE_IDS}, listed here separately because
 * the substrate signal is distinct (the file may carry an `<html>`
 * envelope itself but its semantics depend on framework-injected
 * content), and (b) the event-handler rule {@link
 * import("../rules/keyboard/handler-missing.ts").handlerMissing}
 * whose evidence model fires on bare `<button onClick>` /
 * `<div onClick>` JSX-style emits but cannot see listener wiring the
 * Astro island runtime attaches client-side.
 *
 * Why a separate set rather than reusing `FRAGMENT_DOWNGRADE_RULE_IDS`:
 * the fragment classification names "no `<html>`/`<body>` root" — a
 * structural property of the parsed AST. The astro-islands signal
 * names "the component-script and imported components are unrendered"
 * — a routing-decision property of the substrate that holds even when
 * the file's parsed AST DOES carry an `<html>` root. The two
 * classifications are orthogonal; an `.astro` file may be a fragment,
 * an envelope-carrier, or both, and its rules must downgrade in
 * either case. The reason codes also differ:
 * `fragment-input-no-document-envelope` vs the new
 ...
 * structural accumulator and the scan-assembly seam wants a pure
...
/**
 * Companion of {@link applyFragmentInputAdjustment} on the
 * astro-islands axis. Downgrades a rule's `coverageConfidence` to
 * `"medium"` with
 * `coverageConfidenceReason: "astro-islands-unrendered-static-only"`
 * when at least one of its eligible files is in
 ...
 * Why `"medium"` and not `"low"`: the rule did run, eligibility was
 * met, the file parsed cleanly. The honest signal is "evidence horizon
 * was bounded by the substrate's unrendered-island routing" — a peer
 * to the `fragment-input-no-document-envelope` and
 * `cross_file_*_resolution_not_attempted_by_rule` downgrade shape, not
 * the parse-error invisibility shape. A `"low"` downgrade would
 * conflate this case with the parse-error case (file invisible /
 * partially-parsed), which is a stronger statement than the substrate
 * warrants.
 *
 * Precedence: when {@link applyParseErrorAdjustment},
 * {@link applyScssUnresolvedVariablesAdjustment}, or
 * {@link applyFragmentInputAdjustment} already stamped a non-astro
 ...
 * adjuster runs last in the cascade so the fragment classification
 * (which may also fire on a layout-style `.astro` file lacking an
 * `<html>` root) wins when both apply, since fragment classification
 * names a stronger structural property of the parsed AST.
 *
