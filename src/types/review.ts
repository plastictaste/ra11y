...
export interface ReviewCandidate {
  ...
  /**
   * Structured codes naming the evidence-quality limitations a downstream
   * consumer should weigh when reading this candidate's `priority` /
   * `confidence` signal. Mirrors the same-named field on the rule
   * surface ({@link Violation#couldBeWrongBecause}) and obeys the same
   * present-when-meaningful contract: the field is omitted entirely
   * when no limitation applies, never emitted as `[]` (per CLAUDE.md
   * §1 "Ambiguous field shapes are dishonest").
   *
   * Canonical case shipped today:
   *   - `"minified_vendor_no_sourcemap"` — the cited file carries
   *     `vendorPathHint: true` AND is also classified by the
   *     scan-time build-artifact pass (`scannedBuildArtifacts`).
   *     The combination of "looks like a vendor distribution" plus
   *     "the build-artifact classifier confirms generated bytes"
   *     means the finder's predicate (e.g. a single-letter identifier
   *     in a `setTimeout` call) cannot be honestly resolved without
   *     a sourcemap. The shared priority resolver drops attention
   *     budget to `"low"` on this co-occurrence and stamps the code
   *     here so the downstream agent reads BOTH the budget signal AND
   *     the predicate-strength concession via the same response.
   *
   * Per `docs/kb/architecture/ai-first-consumer.md` "Reason / priority /
   * fix-description must agree across all three channels", and the
   * extension thereof on per-finding confidence: a low-attention-budget
   * candidate must name what it could be wrong about so the agent's
   * dismissal path is "verify the evidence-quality concession" rather
   * than guessing why the priority dropped. Strictly additive — never
   * gates suppression, never alters which criteria attach.
   */
