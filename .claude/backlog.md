...
- [ ] **V1-AUTOCOMPLETE-MISSING-FINDING-ID-COLLISION** — `forms/autocomplete-missing` exhibits the same `findingId` collision pattern as `forms/placeholder-as-label` did before the 2026-04-27 sibling-collapse fix: 4 entries on identical `<input type="email" placeholder="Email">` siblings share one `findingId`. The `_label-sibling-collapse.ts` helper already exists; apply it to the autocomplete-missing emission path so ≥3-sibling clusters collapse to one canonical finding with `siblingInstances`. Per AI-first doctrine "ambiguous field shapes are dishonest" — id collisions are the worst case. Surfaced by the duplicate-finding-ids specialist as an out-of-scope adjacent finding.
