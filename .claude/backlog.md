...
- [ ] **V1-MCP-SUBPROCESS-TEST-DEFLAKE** — `verify_flaky_mcp_subprocess` has fired on every turn of the 2026-05-02 9-turn /continue run (12+ occurrences in 20-turn meta-reviewer window), structurally resistant to two harness patches (203b18a9 integrator.md, 7a836abd dispatch-template.md, both `no_effect`). Root cause: 3 parallel worktree specialists each spawn MCP subprocesses for `tests/integration/mcp-sampling.test.ts`, `tests/integration/mcp-roots.test.ts`, `tests/integration/mcp-protocol.test.ts`, and the consistency tests in `tests/integration/mcp-consistency/scan-time-warnings-cross-surface.test.ts` + `coverage-checklist-consistency.test.ts` — concurrent startup races the 10s `roots/list` and `sampling/createMessage` timeouts. Each retry cycle costs ~30–60k tokens × 2–3 retries × 3 specialists ≈ 300–500k tokens per turn. Closure paths to evaluate (any one suffices): (a) sequential subprocess boot in the test harness via a process pool with a single startup mutex; (b) raise the per-test timeout from 10s to 30s for these specific MCP tests under a `RA11Y_VERIFY_CONCURRENT=1` env var that worktree specialists set; (c) split the MCP integration tests into a separate `bun test --preload <serial>` invocation that runs serially while the rest run parallel. ADR before code: characterize the subprocess startup-ordering precisely so we don't over-fix. Per AI-first doctrine internally — the harness lessons live in `.claude/skills/continue/SKILL.md` lock-out filter (commit efaed6ad).
