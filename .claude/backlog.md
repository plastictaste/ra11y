...
- [ ] **Q9-RULESBYEXTENSION-DUP-PAYLOAD** `coverage.rulesByExtension` and `coverage.rulesFiredByExtension` ship as identical 6-10KB payloads after rename — both populated identically; deprecated_field warning fires but duplicate ships. Same shape as the closed Q9-DUPLICATE-ID-AND-CRITERIONID-AFTER-RENAME at a different field. Fix: drop the deprecated field at alias-removal; short-term dedupe at serialization.
