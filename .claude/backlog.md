...
- [ ] **V1-CERT-VERSION-CONST** Six compliance artifacts hard-code `"0.0.0"`: `src/output/formatters/sarif.ts:26 TOOL_VERSION`, `src/reports/vpat.ts:64 EVALUATOR`, `src/engine/baseline.ts:135 ra11yVersion`, `src/cli/help.ts:7 VERSION`, `src/output/formatters/json.ts:17 version`, `src/output/formatters/terminal.ts:33 VERSION`. Agent formatter + MCP serverInfo say `"0.1.0"` (`src/output/formatters/agent.ts:28`, `src/mcp/server.ts:88`). Every v1.0 scan ships identifying ra11y as v0.0.0. Generate `src/version.ts` from `package.json.version` at build time; consume everywhere. Cheapest win; land first. Small.
