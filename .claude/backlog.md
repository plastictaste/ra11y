...
- [ ] **V1-CERT-ATTEST-INTEGRITY** `src/config/attestation-store.ts` is plain JSONL with caller-supplied `attestedAt`/`by` and no per-record integrity. Trivial to backdate, reorder, delete with `vim`. Decision required: (a) ADR for a per-record SHA-256 hash chain OR (b) ship `ra11y attestations verify` subcommand that confirms the ledger matches `git show HEAD:.ra11y/attestations.jsonl` and `attestedAt` values are ≤ their ancestor commit date. Option (b) is lighter and exploits git as the existing trust root — prefer unless attackers are assumed pre-commit. Write the ADR before coding. Medium.
...
- [ ] **V1-CERT-VPAT-25** `src/reports/vpat.ts:2,256` emits "VPAT 2.4" header; current ITIC template is VPAT 2.5 Rev (WCAG 2.2), with different chapter/FPC layout. Also (a) hardcoded `EVALUATOR = "ra11y v0.0.0"` (closed by V1-CERT-VERSION-CONST), (b) remarks column references internal tooling ("See the terminal or JSON report for file locations") — procurement-unfriendly, (c) `VpatReport` type lacks Product Name / Contact / Evaluation Methods header fields, (d) "Not Applicable" element-presence detection documented in `docs/certification/vpat-mapping.md:55-61` but `buildEntry` (line 138-194) never invokes it. Target VPAT 2.5 Rev layout + accept user-supplied product-metadata inputs + rewrite remarks for auditor audience. Medium.
- [ ] **V1-CERT-E2E-TEST** No integration test exercises scan → attest → `conformance_statement` → `verifyConformanceBundle`. Add `tests/integration/conformance-e2e.test.ts` with four scenarios: (a) happy-path signed-bundle round trip, (b) stale-attestation triggers `"stale-attestation"` blocker (V1-CERT-STALE-ATTEST), (c) source file modified after attest invalidates signature, (d) tampered `.ra11y/attestations.jsonl` detected by `attestations verify` (V1-CERT-ATTEST-INTEGRITY option b) or signature mismatch. Gates the other seven blockers — land last. Medium.
