...
- [ ] **V1-CERT-CLI-PATH** No CLI path for the certification workflow: `ra11y attest` and `ra11y conformance [--verify]` subcommands do not exist (`src/cli/commands/` lists `attestations prune` only). Auditors without an MCP host are locked out. Add both commands wired through `src/cli/exit-codes.ts ExitCode` enum. `ra11y attest` wraps the store primitive (required reason, structured-error on bare); `ra11y conformance` wraps `buildConformanceStatement` + `renderConformanceMarkdown`; `ra11y conformance --verify` wraps `verifyConformanceBundle`. Depends on V1-CERT-STATEMENT-SIGN. Medium.
