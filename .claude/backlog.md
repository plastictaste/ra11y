...
- [ ] **V1-CERT-STATEMENT-SIGN** `src/mcp/tool-conformance-statement.ts:167` never passes `signing` into `buildConformanceStatement`, so `signature` is always `undefined`. The signed-bundle flow advertised in `docs/conformance.md:214-219` is unreachable via MCP. Wire `commitHash` (`src/utils/git.ts:119 headSha`), attestations, configFingerprint + extend fingerprint to cover file-content manifest, resolved config (not just standards/level), and tool version. Depends on V1-CERT-STATEMENT-SHAPE. Medium.
