...
- [ ] **V1-MCP-SUBPROCESS-FLAKE-INVESTIGATE** — **Prerequisite to V1-MCP-SUBPROCESS-TEST-DEFLAKE.** Characterize the actual mechanism behind `verify_flaky_mcp_subprocess` before choosing a fix. The flake re-fired twice during the 2026-05-02 stop-hook turn alone, costing tokens to re-run verify after the user requested stop — empirical evidence that the lock-out + harness patches don't keep tokens off the meter, only the source-level deflake will. **What is observable from the failure output**: (a) the canonical error is `<method> request timed out after 10000ms` thrown from `src/mcp/outbound.ts:56` where `<method>` is always `sampling/createMessage` or `roots/list`; (b) `outbound.ts` is the SERVER-to-CLIENT request rail — the server (under test) sent a JSON-RPC request to its connected client (the test harness) and waited 10s for a reply that never came; (c) the failure is non-deterministic — the same test passes cleanly in isolation in <1s and re-running the full suite often passes; (d) the failure clusters around five specific tests that each spin a real MCP subprocess via stdio (`mcp-sampling.test.ts`, `mcp-roots.test.ts`, `mcp-protocol.test.ts`, `mcp-consistency/scan-time-warnings-cross-surface.test.ts`, `mcp-consistency/coverage-checklist-consistency.test.ts`); (e) the bun output sometimes shows the timeout as an "unhandled error between tests" with no failed test attached, suggesting the request fires from a previous test and times out into a later test's window; (f) reproduction concentrates under load — single-worktree full-verify hits it; 3-worktree concurrent verify hits it nearly every time. **Hypotheses to test in order, each falsifiable**: (1) the test harness's client-side handler for `sampling/createMessage` / `roots/list` is registered AFTER the server sends the first request, so the request enters an unregistered-method state that times out instead of error-replying — instrument the test harness to log every inbound request and verify the registration-vs-request order; (2) test teardown does not await pending outbound requests before killing the subprocess, so a request fired in test N times out into test N+1's window — instrument outbound.ts to log pending-request count at teardown and check for nonzero; (3) the test harness shares an MCP subprocess across tests via module-scope state, and concurrent test files reach the same subprocess from parallel workers — check whether tests/integration/mcp-*.ts use a global subprocess fixture vs per-test spawn; (4) bun's parallel test runner introduces stdio buffering that delays JSON-RPC frame delivery beyond the 10s window under concurrent file IO — measure inbound-frame-arrival latency vs `Date.now()` in outbound.ts under concurrent verify load. **Deliverable**: an ADR (`docs/adr/00NN-mcp-subprocess-flake-rootcause.md`) naming the confirmed mechanism with reproduction steps, plus a one-paragraph recommendation pointing at exactly one of V1-MCP-SUBPROCESS-TEST-DEFLAKE's three closure paths (or a different one if investigation reveals a fourth). Token cost so far per the 2026-05-02 9-turn run: ≥3M tokens of retry overhead. **This item should be the next /continue pick; until it lands, V1-MCP-SUBPROCESS-TEST-DEFLAKE is premature.**
- [ ] **V1-MCP-SUBPROCESS-TEST-DEFLAKE** — Source-level fix following V1-MCP-SUBPROCESS-FLAKE-INVESTIGATE. Three closure paths to evaluate against the ADR's confirmed mechanism: (a) sequential subprocess boot in the test harness via a process pool with a single startup mutex; (b) raise the per-test timeout from 10s to 30s for these specific MCP tests under a `RA11Y_VERIFY_CONCURRENT=1` env var that worktree specialists set; (c) split the MCP integration tests into a separate `bun test --preload <serial>` invocation that runs serially while the rest run parallel. The investigation may surface a fourth path (e.g. fix the registration-vs-request race, or await pending outbound requests at teardown). Estimated saving: ~300–500k tokens per /continue turn. Lock-out lifts from `.claude/skills/continue/SKILL.md` once recurrence rate returns to baseline (per the lock-out-set entry added in commit efaed6ad).
- [ ] **V1-VERIFY-PRECOMMIT-FAST-FOR-SPECIALIST** — Until V1-MCP-SUBPROCESS-TEST-DEFLAKE lands, add `verify:precommit:fast` that excludes the known-flaky integration tests for /continue specialist use only. The integrator continues to run full `bun run verify` post-cherry-pick, so the safety net for genuine MCP regressions is intact — only the specialist's pre-commit verify trades coverage breadth for retry-loop avoidance. Implementation: add a `--exclude-flaky` flag to `scripts/verify.ts`'s test step that filters the named test paths (mcp-sampling.test.ts, mcp-roots.test.ts, mcp-protocol.test.ts, scan-time-warnings-cross-surface.test.ts, coverage-checklist-consistency.test.ts), expose as `verify:precommit:fast` in package.json, then patch `.claude/skills/continue/dispatch-template.md` §4 to reference the fast variant. Estimated saving: ~300–500k tokens per turn during the V1-MCP-SUBPROCESS-TEST-DEFLAKE interim. Pairs with V1-MCP-SUBPROCESS-TEST-DEFLAKE (this is the temporary lever; that's the durable fix) — close this when the deflake lands.
