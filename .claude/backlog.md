...
- [ ] **V1-CERT-STALE-ATTEST** `src/reports/conformance.ts:386-405 classifyBlocker` never consults the git-staleness probe that `src/reports/attestation-surface.ts` already computes. An attestation recorded at commit A against a file changed at commit B silently stays `status: pass` and emits zero blockers. Wire the staleness probe into `buildConformanceStatement`; add `"stale-attestation"` and `"missing-process-config"` (also documented but unimplemented, ADR 0016) variants to `ConformanceBlockerReason` (line 66-70). Small.
