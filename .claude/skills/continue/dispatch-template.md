...
**Close your backlog item in the work commit.** Before returning, your work is not complete unless the `- [ ] **<ID>**` line for the pick is deleted from `.claude/backlog.md` and the closing commit message carries a `Closes: <ID>` (shipped) or `Drops: <ID>` (rejected/superseded) trailer per CLAUDE.md §9 #7. Stage the backlog edit alongside your last logical commit, or in a sibling `chore(backlog):` commit if your feat/fix landed first. Specialists who skip the closure force the integrator to do it on their behalf, costing an extra round-trip and burning context. If the worktree-guard hook blocks the edit, return `signals[].code: backlog_tickoff_blocked` (stable signal name retained for ledger continuity) with the hook output in `evidence`; do not silently leave the line.
...
**Do NOT self-attribute pre-existing main debt — fix it or return `blocked`.** Earlier guidance allowed `signals[].code: verify_red_pre_existing_on_main` claims if backed by `git show main:<path>` evidence. Six successive false attributions across the 2026-05-02 run (often with detailed but stale-looking evidence) cost the integrator tokens to re-verify each time. A plausible mechanism: stale worktree `main` ref pointing at the fork-point SHA rather than live HEAD, making specialist-side `git show main:<path>` evidence structurally unreliable. New contract: when `verify:precommit` reports a red lint/format/type failure, **fix it** before returning `verifyPrecommit: "ok"`, OR return `blocked: "verify-red: <one-line summary>"`. Do NOT emit `verify_red_pre_existing_on_main` from a worktree-isolated dispatch under any circumstance — the integrator runs `git checkout main && bun run verify` automatically before every cherry-pick batch, and that plain-main verify is the only authoritative source on whether main is dirty. Eliminating this self-attribution channel removes the false-claim verbosity entirely; the integrator's verify is the safety net for the rare case main genuinely is dirty.
