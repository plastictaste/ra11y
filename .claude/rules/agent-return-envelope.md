...
- **Integrator `errors[]` strings** (per `integrator.md` §6): `dirty_main:`, `dirty_main_unrelated_wip:`, `cherry_pick_conflict:`, `verify_red:`, `cross_pick_interaction:`, `unknown_state:`, `branch_empty_sibling_has_work:` (when `note` carries the same signal). `dirty_main_unrelated_wip:` distinguishes a concurrent-writer-dirtied main (dirty paths outside any pick's inferred scope) from a worktree-escape-dirtied main (`dirty_main:`, dirty paths overlap pick scope).
...
- **Meta-reviewer self-findings** (per `meta-reviewer.md` §3a, §3b, §8, §8a): `harness_patch_verify_red` (the patch the agent tried to land failed `bun run verify`), `correlated_signals` (a co-occurring pair where one half couldn't be auto-patched and the routing tree escalated), `harness_patch_no_effect` (a prior `chore(meta):` patch did not reduce its target signal's rate over the post-patch window), `patch_aged_out` (a prior patch passed beyond the ledger window without enough evidence for a verdict), `memory_dir_absent` (the user-local memory directory was missing when §8a tried to consolidate; retirement skipped). Emitted by the agent into its own `findings[]` for next-turn occurrence-count purposes.
