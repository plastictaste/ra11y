...
- **Meta-reviewer self-findings** (per `meta-reviewer.md` §3a, §3b, §8): `harness_patch_verify_red` (the patch the agent tried to land failed `bun run verify`), `correlated_signals` (a co-occurring pair where one half couldn't be auto-patched and the routing tree escalated), `harness_patch_no_effect` (a prior `chore(meta):` patch did not reduce its target signal's rate over the post-patch window), `patch_aged_out` (a prior patch passed beyond the ledger window without enough evidence for a verdict). Emitted by the agent into its own `findings[]` for next-turn occurrence-count purposes.
