...
| `file-not-found` | `scan_file`, `suppress` | The target file does not exist at the given path. `scan_file` emits this when the path-exists pre-check fails (distinct from `file-unsupported`, which names the extension-not-parseable branch — see Q-SHARED-SCAN-FILE-ERROR-DISCRIMINATION). | Verify the file exists before calling. Relative paths resolve against `cwd`. |
| `file-unsupported` | `scan_file`, `suggest_fix`, `apply_fix`, `suppress` | The file extension is not in the parser registry (`PARSEABLE_EXTENSIONS` in `src/utils/path.ts`). `scan_file` emits this only after the path-exists pre-check passes, so the code names the extension filter unambiguously; `suggest_fix`/`apply_fix` still conflate extension and readability in this code. | Pass a supported file type. `scan_file`'s remediation enumerates the live parser registry. |
| `file-read-failed` | `scan_file`, `apply_fix`, `suppress` | The file exists and has a supported extension, but reading or parsing it raised an I/O or parse error. `details.cause` carries the underlying message. | Inspect `details.cause`; fix permissions or encoding issues, then retry. |
