...
| contrast/enhanced | guidance | context-aware | foreground source, background source, measured ratio, required minimum, large-text flag | Shared `buildContrastSuggestion` inlines both color tokens, the current ratio, the required threshold, and the multiplicative gap. Picking a passing color is a design decision (brand palette, theme variants) ra11y has no context for; the suggestion describes the gap and points at the design-system palette rather than prescribing a specific replacement. |
...
| contrast/non-text | guidance | context-aware | CSS property, fg source, bg source, measured ratio | `buildSuggestion(prop, fgSource, bgSource, ratio)` — per-property boundary advice with the failing pair, ratio, and gap inlined. No prescribed replacement color. |
